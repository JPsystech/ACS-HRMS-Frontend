...
export default function CompOffPage() {
  ...
  const fetchData = async () => {
    ...
    const isGlobal = ["HR", "ADMIN", "MD"].includes(user.role)
    const isApprover = ["MANAGER", "VP"].includes(user.role)

    if (isGlobal) {
      // Global roles can see all employees
      try {
      ...
      // Approvers see only reportees
      try {
        const data = await api.get<Employee[]>("/api/v1/employees")
        // Note: For now, we use a simple check. 
        // In a real hierarchical setup, the backend should provide a /subordinates endpoint.
  ...
  const fetchRequests = async () => {
    ...
    setApiError("")
    try {
      const isApproverRole = ["HR", "MANAGER", "VP", "ADMIN", "MD"].includes(user.role)
      
      // Use pending endpoint for pending tab (Approvers only)
      // Otherwise use my-requests which is role-scoped
      let endpoint =
        activeTab === "pending" && isApproverRole
  ...
  const canApproveReject = (request: CompoffRequest) => {
    ...
    // Global roles can approve all
    if (["HR", "ADMIN", "MD"].includes(user.role)) return true

    // Manager/VP can approve only if they are the direct manager of the employee
    // or if the request is in their pending list (which backend filters hierarchically)
    if (["MANAGER", "VP"].includes(user.role)) {
      // If we're on the pending tab, all items listed are approvable by current user
      if (activeTab === "pending") return true
      
      // Fallback: check if the employee reports to the user
  ...
  const canViewFilters = ["HR", "ADMIN", "MD", "VP", "MANAGER"].includes(user?.role || "")

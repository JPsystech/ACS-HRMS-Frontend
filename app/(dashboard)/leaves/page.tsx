...
export default function LeavesPage() {
  ...
  const getEmployeeDetails = (employeeId: number) => {
    ...
    if (emp) {
      return {
        ...
        // Prefer backend-provided department.name, else map department_id via departments list
        department:
          emp.department?.name ||
  ...
  const renderActionSummary = (leave: LeaveRequest) => {
    ...
    return (
      <div className="flex flex-col gap-1 text-[11px] leading-snug min-w-[180px]">
        <div className="font-semibold text-foreground flex items-center gap-1.5">
          <span className={
            type === "Approved" ? "text-green-600" : 
            type === "Rejected" ? "text-red-600" : "text-orange-600"
          }>
            {type}
          </span>
          <span className="text-muted-foreground font-normal">by</span>
          <span className="truncate max-w-[100px]" title={by}>{by}</span>
          {role && (
        ...
        {remark ? (
          <div 
            className="mt-1 p-1.5 bg-muted/30 rounded border border-muted/50 text-foreground italic max-h-[60px] overflow-y-auto scrollbar-thin"
            title={remark}
          >
          ...
          <div className="text-muted-foreground/60 italic mt-0.5">Remark: —</div>
        )}
      </div>
    );
  };
  ...
  const canApproveReject = (leave: LeaveRequest) => {
    ...
    // ADMIN/MD/VP can approve any pending leave (global authority)
    if (user.role === "ADMIN" || user.role === "MD" || user.role === "VP") return true

    // HR can approve any pending leave (HR ops)
    if (user.role === "HR") return true

    // MANAGER and other team roles: only if they are the designated approver
  ...
  const canCancel = (leave: LeaveRequest) => {
    ...
    if (leave.status === "PENDING") {
      // Employee can self-cancel their own pending leave if it's in the future
      if (isSelf) {
        return fromDate > today
      }
      // Admin, MD, HR, VP, MANAGER can cancel pending leaves for cleanup
      return role === "ADMIN" || role === "MD" || role === "HR" || role === "VP" || role === "MANAGER"
    }
    
    if (leave.status === "APPROVED") {
      // HR can cancel any approved leave (including past ones)
      if (role === "HR") return true

      if (isSelf) {
        // Employee can self-cancel approved leave if it's today or in the future
        // (Note: backend also checks attendance for today)
        return fromDate >= today
      }

      // Admin, MD, VP, MANAGER can cancel future/today approved leaves
              ...
              <AnimatedTable>
                ...
                <TableBody>
                  ...
                  <TableCell>
                    <Badge 
                      className={
                        leave.status === "PENDING" ? "bg-yellow-100 text-yellow-800 border-yellow-200 animate-pulse" :
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedLeave?.status === "PENDING" 
                ? "Are you sure you want to cancel your pending leave request?"

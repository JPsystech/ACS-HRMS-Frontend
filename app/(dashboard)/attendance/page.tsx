...
import { Clock, Edit, Loader2, MapPin, AlertTriangle, Plus, RefreshCw } from "lucide-react"
...
export default function AttendancePage() {
  ...
  const fetchEmployees = async () => {
    try {
      if (user?.role === "HR" || user?.role === "ADMIN") {
        ...
        // Use full tree so VP can filter/search across indirect reports
  ...
  const fetchSessions = async () => {
    ...
    try {
      if (activeTab === "today") {
        ...
        // Use /today-scope for Managers to see their team
  ...
  return (
    ...
    >
      {user && SUMMARY_ROLES.includes(user.role) && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="rounded-lg border p-4">
            <h3 className="text-sm font-medium text-muted-foreground mb-1">Team Attendance Today</h3>
            {teamSummary ? (
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  <div className="text-2xl font-semibold text-muted-foreground">{teamSummary.team_total}</div>
                  <div className="text-xs">Total</div>
                </div>
                <div>
                  <div className="text-2xl font-semibold text-green-600">{teamSummary.present_today}</div>
                  <div className="text-xs">Present</div>
                </div>
                <div>
                  <div className="text-2xl font-semibold text-orange-600">{teamSummary.not_punched}</div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <>
                {reportsSubTab === "sessions" ? (
                  ...
                  <>
                    ...
                    <Button variant="outline" size="sm" onClick={fetchNotPunchedToday} disabled={notPunchedLoading}>
        ...
        <TabsContent value="reports" className="mt-4">
          <Tabs value={reportsSubTab} onValueChange={(v) => setReportsSubTab(v as any)}>
            <TabsList className="grid w-full max-w-md grid-cols-2">
              <TabsTrigger value="sessions">Sessions</TabsTrigger>
              <TabsTrigger value="not_punched_today">Not punch today</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="not_punched_today" className="mt-4">
              {notPunchedLoading ? (
                <Skeleton className="h-64 w-full" />
              ) : notPunchedEmployees.length === 0 ? (
                <EmptyState
                  icon={Clock}
                  title="No employees"
                  description="Everyone has punched today (or no employees matched filters)."
                />
              ) : (
                <AnimatedTable>
                  <TableHeader>
                  ...
                  <TableBody>
                    {notPunchedEmployees.map((e) => (
                      <AnimatedTableRow key={e.employee_id}>
...
function AttendanceTable({
  ...
  const handleResolve = (s: AdminAttendanceSessionDto, action: "KEEP_LEAVE" | "REVOKE_LEAVE") => {
    ...
    api.post(`/api/v1/admin/attendance/${s.id}/resolve-conflict`, {
      ...
      setResolveRemarks("")
      // The parent component should ideally provide a refresh function, but here we can just reload
  ...
  return (
      ...
      <TableBody>
        {sessions.map((s) => (
          <AnimatedTableRow key={s.id}>
            ...
            <TableCell>
              <Badge variant={s.status === "OPEN" ? "default" : s.status === "LEAVE_CONFLICT" ? "destructive" : "secondary"}>
                {s.status === "LEAVE_CONFLICT" ? "Conflict" : s.status}
              </Badge>
            </TableCell>
            {showActions && (
              <TableCell className="text-right">
                <div className="flex justify-end gap-1">
                  {s.status === "LEAVE_CONFLICT" && (
                    ...
                    >
                      <AlertTriangle className="h-4 w-4 mr-1" /> Resolve
                    </Button>
                  )}
    ...
    <Dialog open={resolveOpen} onOpenChange={setResolveOpen}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Resolve Leave-Attendance Conflict</DialogTitle>
          <DialogDescription>
            {selectedForResolve?.employee_name} has an approved leave today but marked attendance.
          </DialogDescription>
        </DialogHeader>

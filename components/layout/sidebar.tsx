...
import { cn, canAccessTeamModulesOrHr } from "@/lib/utils"
import {
  LayoutDashboard,
  Building2,
  Users,
...
const navItems: NavItem[] = [
  ...
  {
    title: "Payroll",
    href: "/admin/payroll/attendance-preview",
    icon: WalletCards,
    roles: ["HR", "ADMIN"],
  },
  {
  ...
  {
    title: "Salary Master",
    href: "/admin/payroll/salary-master",
    icon: WalletCards,
    roles: ["HR", "ADMIN"],
  },
  {
    title: "Payroll Reminders",
    href: "/admin/payroll/reminders",
    icon: Mail,
    roles: ["HR", "ADMIN"],
  },
  {
    title: "Payroll Email Settings",
    href: "/admin/payroll/email-settings",
    icon: Mail,
    roles: ["HR", "ADMIN"],
  },
  {
...
export function Sidebar({ initialCollapsed = false, onNavigate }: SidebarProps) {
  ...
  const filteredNavItems = mustChangePassword
    ? [
        {
    ...
    : navItems.filter((item) => {
        ...
        // Allow VP/MD (and any rank<=4) into manager/team modules when those items
        // were historically guarded by MANAGER only.
        if (item.roles.includes("MANAGER")) {
          return canAccessTeamModulesOrHr(user as any)
        }
        return false
      })
